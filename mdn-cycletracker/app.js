...
newPeriodFormEl.addEventListener("submit", (event) => {
  ...
  // since everything is client-side. (privacy)
...
function checkDatesInvalid(startDate, endDate) {
  ...
  // the pipes are combining the requirements
...
function renderPastPeriods() {
  ...
  const pastPeriodHeader = document.createElement("h2"); //thats crazy you can create tag elements in javascript
  ...
  periods.forEach((period) => {
    const periodEl = document.createElement("li"); //creating a seperate list element for every period
...
function formatDate(dateString) {
  ...
  return date.toLocaleDateString("en-AU", { timeZone: "AEST" }); //aus timezone
}
